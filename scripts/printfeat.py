        ...
        for opt_img, sar_img ,labels in tbar:
            ...
            for t in opt.t:
                ...
                for scale in opt.feat_scales:
                    print_feats(path=opt.output_dir, feats_A=fd_A_t, feats_B=fd_B_t, scale=scale, t=t,step=total_step)
        ...
        with torch.no_grad():
            ...
            if mean_val_metrics['cd_f1scores'] > best_metrics['cd_f1scores']:
                ...
                if dist.get_rank() == 0:
                    ...
                    torch.save(state, opt.weight_dir + '/checkpoint_epoch_' + str(epoch) + '.pt')
                if dist.get_rank() == 1:
                    state = {'epoch': epoch,
                             'model': model.state_dict(),
                             'optimizer': optimizer.state_dict()}
