...
class ImageDataset(Dataset):
    def __init__(
        ...
        self.use_yuv_space = use_yuv_space
    ...
    def __getitem__(self, idx):
        ...
        if self.use_lab_space:
            ...
            arr = (arr - np.array([[50., 0., 0.]]).astype(np.float32)) / np.array([[50., 128., 128.]]).astype(np.float32)

        else:
            if self.in_channels == 3:
                if self.use_yuv_space:
                    pil_image=np.array(pil_image).astype(np.float32)
                    pil_image=pil_image/255.0
                    pil_image=color.rgb2yuv(pil_image)
                    pil_image = Image.fromarray(np.uint8(pil_image*255.0+np.array([0,127.5,127.5])))
                else:
            ...
            # arr=np.expand_dims(arr[:,:,1],axis=-1)
