...
def full_path_loader(data_dir):
    train_data = [i for i in os.listdir(data_dir + 'train/optical/') if not
    i.startswith('.')]
    train_data.sort()

    valid_data = [i for i in os.listdir(data_dir + 'test/optical/') if not
    i.startswith('.')]
    valid_data.sort()
    ...
    for img in valid_data:
        val_label_paths.append(data_dir + 'test/OUT/' + img)
...
def full_test_loader(data_dir):
    ...
    test_data = [i for i in os.listdir(data_dir + 'test/optical/') if not
                    i.startswith('.')]
    test_data.sort()
...
def cdd_loader(img_path, label_path, aug):
    ...
    img1 = Image.open(dir + 'optical/' + name).convert('RGB')
    img2 = Image.open(dir + 'SAR3ch/' + name).convert('RGB')
...
def cdd_loader_slic(img_path, label_path, aug):
    dir = img_path[0]
    name = img_path[1]
    ...
    return sample['image'][1], sample['image'][0], sample['image'][2],sample['label']
