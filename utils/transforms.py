...
class ToTensor(object):
    ...
    def __call__(self, sample):
        ...
        img1 = Image.fromarray(np.uint8(img1 * 255.0 + np.array([0, 127.5, 127.5])))

        img1 = np.array(img1).astype(np.float32).transpose((2, 0, 1)) / 127.5 - 1
        img2 = np.array(img2).astype(np.float32).transpose((2, 0, 1)) / 127.5 - 1
        # label = np.array(mask).astype(np.float32) / 255.0
        ...
        # trutth = mask
        # truth = np.where(mask==0, -1, truth)
        # truth = np.where((mask> 0.5) & (mask<1), 0, truth)
...
class ToTensor_slic(object):
    ...
    def __call__(self, sample):
        ...
        img1 = np.array(img1).astype(np.float32).transpose((2, 0, 1)) / 127.5 - 1
        img2 = np.array(img2).astype(np.float32).transpose((2, 0, 1)) / 127.5 - 1
        opt_orig = np.array(opt_orig).astype(np.float32).transpose((2, 0, 1)) / 127.5 - 1
        # label = np.array(mask).astype(np.float32) / 255.0
        # img1 = np.array(img1).astype(np.float32).transpose((2, 0, 1))
        # img2 = np.array(img2).astype(np.float32).transpose((2, 0, 1))
        label = np.array(label).astype(np.float32)
        ...
        img1 = torch.from_numpy(img1).float()
        img2 = torch.from_numpy(img2).float()
        opt_orig = torch.from_numpy(opt_orig).float()
        lbl = torch.from_numpy(lbl).float()

        return {'image': (img1, img2,opt_orig),
                'label': lbl}
