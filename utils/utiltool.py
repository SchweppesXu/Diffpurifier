...
def print_feats(path,feats_A, feats_B, scale, t,step):
    feature_path = '{}/features'.format(path)
    os.makedirs(feature_path, exist_ok=True)

    # img_A = Metrics.tensor2img(train_data[0][0,:,:,:])  # uint8
    # Metrics.save_img(img_A, '{}/img_A.png'.format(feature_path))
    #
    # img_B = Metrics.tensor2img(train_data[1][0,:,:,:])  # uint8
    # Metrics.save_img(img_B, '{}/img_B.png'.format(feature_path))

    # for i in range(feats_A[scale].size(1)):
    t_path = '{}/{}/{}'.format(feature_path,t,step)
    os.makedirs(t_path, exist_ok=True)
    feat_img_A = Metrics.tensor2img(feats_A[scale][0, 1, :, :])  # uint8
    Metrics.save_feat(feat_img_A, '{}/feat_opt_{}_scale_{}_t_{}.png'.format(t_path, 1, scale, t))

    feat_img_B = Metrics.tensor2img(feats_B[scale][0, 1, :, :])  # uint8
    Metrics.save_feat(feat_img_B, '{}/feat_sar_{}_scale_{}_t_{}.png'.format(t_path, 1, scale, t))
...
class FeatureConverter:
    def __init__(self, eta_pos, gamma_clr, offsets):
        ...
        self.offsets = torch.Tensor(offsets).view(1,len(offsets),1,1) #重构为1,5,1,1的张量
    ...
    def __call__(self, feats, nw_spixels, nh_spixels):
        ...
        scales = torch.Tensor((scale_pos,)*2+(scale_clr,)*(c-2))#根据括号里数据创建张量
